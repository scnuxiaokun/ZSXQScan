...
let _dbMode = null; // 'cloud' | 'local' | 'mysql' | 'cloud-mysql' | null

/**
 * 从外部注入集合实例（server.js 调用，避免重复初始化数据库连接）
 */
function injectCollection(collection, mode) {
  if (collection) {
    _configCollection = collection;
    _dbMode = mode || 'injected';
    console.log(`[CookieManager] 注入集合成功 (mode=${_dbMode})`);
  }
}
...
function getConfigCollection() {
  ...
  if (process.env.DB_HOST) {
    ...
    const db = init();
    _configCollection = db.collection('config');
    _dbMode = 'cloud-mysql';
  ...
  try {
    ...
    if (doc.data && doc.data.value) {
      ...
      if (typeof val === 'string') { 
        try { val = JSON.parse(val); val = val.value || val; } catch(e) {} 
      }
      // 兼容：如果解析后还是对象，取 .value
      if (typeof val === 'object' && val !== null) {
        val = val.value || val;
      }
    }
    if (typeof val === 'string' && val.length > 0) {
      console.log(`[CookieManager] 从${_dbMode === 'cloud' ? '云数据库' : _dbMode === 'mysql' ? 'MySQL数据库' : '本地JSON'}读取到 Cookie`);
      return val;
    }
    return null;
  } catch (e) {
...
async function checkCookieStatus() {
  ...
  try {
    ...
    if (doc.data && doc.data.value) {
      ...
      result.lastUpdated = doc.data.updated_at || doc.data.updatedAt || doc.data.createdAt;
    }
  } catch (e) {}
