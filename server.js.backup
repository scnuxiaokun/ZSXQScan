...
try {
  ...
  if (cm.injectCollection) {
    cm.injectCollection(configCollection, hasMySQL ? 'mysql' : (hasCloudEnv ? 'cloud' : 'local'));
...
function parseArticleDetail(apiResponse, topicId) {
  ...
  let content = topic.text ? htmlToPlainText(topic.text) : (topic.text_summary || '');
...
async function fetchArticle(planetUrl, topicId) {
  ...
  if (!targetTopicId) {
    ...
    const latest = topicsResult.topics?.[0] || topicsResult.data?.[0];
    if (!latest?.id) throw new Error('未能获取到最新话题ID');
    targetTopicId = latest.id;
  }
